...
import { saveBranch, subscribeBranches, subscribeOrders } from "@/lib/services/menu";
...
export function AdminShell() {
  ...
  const [section, setSection] = useState<"overview" | "orders" | "business" | "menu" | "themes">(
    "overview"
  );
  ...
  async function handleCreateBranch(event: React.FormEvent) {
    ...
    try {
      ...
      if (newBranch.isPrimary) {
        await Promise.all(
          branches
            .filter((branch) => branch.id !== newBranch.id && branch.isPrimary)
  ...
  async function handleUpdateBranch(event: React.FormEvent) {
    ...
    try {
      if (branchEditor.isPrimary) {
        await Promise.all(
          branches
            .filter((branch) => branch.id !== branchEditor.id && branch.isPrimary)
            .map((branch) => saveBranch({ ...branch, isPrimary: false }))
        );
      }
  ...
  return (
    <div className="relative grid gap-6 xl:grid-cols-[auto_minmax(0,1fr)]">
      ...
      <aside
        className={[
          "flex h-full flex-col rounded-shell border border-line bg-panel p-4 transition-all",
      ...
      >
        <div className="flex items-center justify-between border-b border-line pb-4">
          ...
          <button
            ...
            className="rounded-full border border-line bg-surface p-2 text-text"
        ...
        <div className="mt-4 space-y-3">
          <button
            ...
            onClick={toggleTheme}
            className={[
              "inline-flex items-center gap-2 rounded-full border border-line bg-surface px-4 py-2 text-sm font-semibold text-text",
              sidebarCollapsed ? "justify-center px-0 w-10 h-10" : ""
            ].join(" ")}
          >
            {theme === "dark" ? <SunMedium size={16} /> : <MoonStar size={16} />}
            {!sidebarCollapsed && (theme === "dark" ? "Claro" : "Oscuro")}
          </button>
          ...
          {[
            ...
            return (
              <button
                ...
                className={[
                  "flex w-full items-center gap-3 rounded-card px-4 py-3 text-left text-sm font-semibold transition",
                  active
        ...
        <div className="mt-auto flex items-center justify-between border-t border-line pt-4">
          ...
          <button
            type="button"
            onClick={() => void logout()}
            className="rounded-full border border-line px-4 py-2 text-sm font-semibold text-text"
          >
      ...
      <main className="space-y-6">
        {section === "overview" && (
          <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
            <section className="space-y-5 rounded-shell border border-line bg-panel p-6">
              <div>
                <p className="text-sm uppercase tracking-[0.25em] text-brand">General</p>
                <h2 className="mt-2 text-2xl font-semibold text-text">Nueva sucursal</h2>
              </div>

              <form onSubmit={handleCreateBranch} className="grid gap-4">
                ...
                <label className="flex items-center gap-3 rounded-card border border-line bg-surface px-4 py-3 text-sm text-text">
                  <input
                    type="checkbox"
            ...
            <section className="space-y-5 rounded-shell border border-line bg-panel p-6">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold text-text">Vista general</h2>
              </div>
          ...
          <section className="space-y-5 rounded-shell border border-line bg-panel p-6">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-text">
                {branchEditor ? branchEditor.name : "Selecciona una sucursal"}
              </h2>
            </div>
            ...
            {branchEditor ? (
              <form onSubmit={handleUpdateBranch} className="space-y-5">
                ...
                <div className="space-y-3">
                  <p className="text-sm uppercase tracking-[0.25em] text-brand">Horarios</p>
                  <div className="space-y-3">
                    {(branchEditor.weeklyHours || defaultSchedule()).map((item, index) => (
                      <div
                        key={`${item.day}-${index}`}
                        className="grid gap-3 rounded-card border border-line bg-surface p-4 md:grid-cols-[0.95fr_0.6fr_0.7fr_0.7fr]"
                      >
                        ...
                        <div className="rounded-card border border-line bg-panel px-4 py-3 text-sm text-muted">
                        ...
                        <input
                          ...
                          className="rounded-card border border-line bg-panel px-4 py-3 text-sm text-text outline-none disabled:text-muted"
                        />
        ...
        {section === "menu" && (
          <AdminProductForm branch={selectedBranch} section="menu" onNotify={notify} />
        )}
        {section === "themes" && (
          <AdminProductForm branch={selectedBranch} section="themes" onNotify={notify} />
        )}
